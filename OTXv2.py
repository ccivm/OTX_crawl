...
class OTXv2(object):
    ...
    def __init__(
        ...
        user_agent=None, verify=False, cert=None
    ):
        self.key = api_key
        self.server = server
        self.verify = verify
    ...
    def walkapi_iter(self, url, max_page=None, max_items=None, method='GET', body=None):
        ...
        while next_page_url:
            ...
            if method == 'GET':
                try:
                    print(f"doing_url:{next_page_url}")
                    data = self.get(next_page_url)
                except:

                    parts = next_page_url.split('page=')
                    if len(parts) == 2:
                        page_number = int(parts[1])
                        page_number += 1 # 增加 2
                        # 构建新的 URL
                        new_url = f"https://otx.alienvault.com/api/v1/pulses/subscribed?limit=50&page={page_number}"
                    #next_page_url=input("手动输入:",)
                    print(f"new_url:{new_url}")
                    data = self.get(new_url)
